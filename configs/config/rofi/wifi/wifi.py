...
def confirm_cmd():
    ...
    confirm = subprocess.run(
        [
            "rofi",
            "-theme", f"{dir}/{theme_confirm}.rasi",
            "-theme-str", "window {location: center; anchor: center; fullscreen: false; width: 330px;}",
        ...
        input=confirm_input, capture_output=True, text=True
    )
...
def notify_connection_status(success, ssid):
    ...
    if success:
        subprocess.run(
            ["notify-send", "-i", icon_path, "-t", "5000", "-r", "2593", "Wi-Fi Connected", f"Connected to {ssid}"])
    else:
        subprocess.run(
            ["notify-send", "-i", icon_path, "-t", "5000", "-r", "2593", "Connection Failed", f"Failed to connect to {ssid}"])
...
while True:
    ...
    menu_items.extend([f"{net["icon"]}  {net["signal"]} {
                      net["ssid"]}" for net in known_nets])

    menu_items.extend([f"{net["icon"]}  {net["signal"]} {
                      net["ssid"]}" for net in unknown_nets])
    ...
    chosen = subprocess.run(
        ["rofi", "-dmenu", "-p",
            f"Network ({get_wifi_prompt()})", "-theme", f"{dir}/{theme}.rasi"],
    ...
    if chosen.returncode == 0:
        ...
        if selected in ["󱚶  Disable Wi-FI", "󱚺  Enable Wi-FI"]:
            if "Disable" in selected:
                ...
                subprocess.run(["notify-send", "-i", f"{home}/.local/share/icons/Papirus/24x24/panel/network-wireless-offline.svg",
                               "-t", "5000", "-r", "2593", "Wi-Fi Disabled", "Wi-Fi is offline now"])

            else:
                # nmcli then notify-send
                subprocess.run(["nmcli", "radio", "wifi", "on"])
                subprocess.run(["notify-send", "-i", f"{home}/.local/share/icons/Papirus/24x24/panel/network-wireless-on.svg",
                               "-t", "5000", "-r", "2593", "Wi-Fi Enabled", "Wi-Fi is online now"])
            continue
        ...
        if selected_ssid in known_networks:
            if confirm_cmd():
                ...
                if (wifi_info["status"] == "connected" and wifi_info.get("ssid") == selected_ssid):
                    print(f"Already connected to {
                    ...
                    notify_connection_status(
                        result.returncode == 0, selected_ssid)
            else:
            ...
            if confirm_cmd():
                ...
                if password:
                    ...
                    if result.returncode == 0:
                        ...
                        subprocess.run(
                            ["nmcli", "con", "delete", selected_ssid])
